...
user = data['user_name'].to_list()

pattern = r"xva\.(?P<env>[^\.]+)\.(?P<role>[^\.]+)\.(?P<index>[^\.]+)\.(?P<disk>[^\.]+)\."
...
def fillin(x):
    if isinstance(x, str):
        if x[0:3] == "xva":
            str_new = re.findall(pattern, x)
            role.append(str_new[0][1])
            index.append(str_new[0][2])
            return True
        else:
            role.append('')
            index.append('')
...
a = list(filter(fillin, user))

# print(role)
# for i in range(0, len(user)):
#     if isinstance(user[i], str):
#         if user[i][0:3] == "xva":
#             str_new = re.findall(pattern, user[i])
#             role.append(str_new[0][1])
#             index.append(str_new[0][2])
#             i = i + 1
#         else:
...
data = data.pivot_table(index=['role', 'index', 'env'], columns=['week_number', 'week_day'], values='cost')
print(data)

writer = pd.ExcelWriter('dimo_version_new.xlsx')
